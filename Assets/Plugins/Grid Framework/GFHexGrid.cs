...
public class GFHexGrid : GFLayeredGrid {
	...
	/// <c>CompactRectangle</c> is similar with the odd-numbered colums one hex
	/// shorter.
	...
	///	value may not be less than 0.1 (please contact me if you really need
	///	lower values).
	/// </value>
	public float radius {
		get { return _radius; }
		set { SetMember<float>(value, ref _radius, restrictor: Mathf.Max, limit: 0.1f); }
	}
	...
	/// coordinaate is simply which layer of the grid is on, relative to the
	...
	private Vector4 HerringToCubic(Vector3 herring, bool up) {
		...
		index = Mathf.FloorToInt(herring[idxS[0]]); // odd or even (idxS[0] means X-axis for pointy sides and Y-axis for flat sides)
		if (hexSideMode == HexOrientation.PointySides) {
			x = herring[idx[0]];
			if (up) {
				if ((index & 1) == 0) { // even
	...
	/// 
	/// Aligns a poistion vector to the grid by positioning it on the centre of
	/// the nearest face. Please refer to the user manual for more information.
	/// The parameter lockAxis makes the function not touch the corresponding
	/// coordinate.
	...
	/// nearest multiple of the grid’s radius and depth.  The @c lockAxis
	...
	/// This method is only intended for internal use to contribue lines to the
	/// draw points.
	...
	/// point but two layers. it's inteded for layer lines.
	...
	private void drawPointsCountRect(ref int countX, ref int countY, ref int countZ, ref Vector3 from, ref Vector3 to, bool compact = false) {
		...
		//    _   _
		//  _/ \_/ \_  (1) Every top hex needs a N lin.
		// / \_/ \_/ \ (2) Every right hex needs a NE and SE lin.
		...
		// are swapped.
		//  _   _   _
		// / \_/ \_/ \  Clipped recangles are the same both up- and downwards. We'll
		...
		if (compact) {
			// Firest count the number of odd columns. There are two cases:
		...
		// respective axes we have to apply the inverese of the `idx` (pointy
	...
	private void DrawPointsCountCircle(ref int countX, ref int countY, ref int countZ, ref Vector3 from, ref Vector3 to) {
		 ...
		 * and and angle. The distance is from the central hex, if it is 0 the
	...
	private void drawPointsCalculateRect(Vector3[][][] points, int[] amount, Vector3 from, Vector3 to, bool compact, bool upwards) {
		 ...
		 * 
		 * We need three hex coordinates. The first will ve be column hex, it
	...
	private void drawPointsCalculateCircle(Vector3[][][] points, int[] amount, Vector3 from, Vector3 to) {
		...
		//angle[1]  = Mathf.Min(angle[1], angle[0] + 6);
