...
[System.Serializable] public abstract class GFGrid : MonoBehaviour {
	...
	#region Matrices
	/// @internal
	/// <summary>Flag that tells us when we need to update the matrices before carrying out matrix operations.</summary>
	///
	/// <para>Whenever something is done to the grid that cahnges the
	/// coordinate conversion matrices set this flag to <c>true</c>. After the
	/// matrices have been updated set it back to <c>false</c>.</para>
	...
	/// jagged one.  The outher dimension is always 3 and stands for the three
	...
	/// 
	/// The colours are stored as three separte entries, corresponding to the
	...
	/// otherwise this default material will be used:
	/// <code>
	...
	protected static Material DefaultRenderMaterial { get { return new Material(DefaultShader); } }
	#endregion
	
	#region helper values (read only)
	/// The normal X-, Y- and Z- vectors in world-space.
	protected static Vector3[] Units { get { return new Vector3[]{Vector3.right, Vector3.up, Vector3.forward}; } }
	#endregion
	...
	/// <param name="worldPoint">Point in world space.</param>
	/// 
	/// Returns the position of the nerest vertex in grid coordinates from a
	...
	/// 
	/// The resulting position depends on <cref name="AlignVector3"/>, so please look up how that method works.
