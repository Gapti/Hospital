...
/// 
/// A grid based on cylindrical coordinates. The caracterizing values are <see
/// cref="radius"/>, <see cref="sectors"/> and <see cref="depth"/>. The angle
/// values are derived from <see cref="sectors"/> and we use radians
/// internally. The coordinate systems used are either a grid-based coordinate
/// system based on the defining values or a regular cylindrical coordinate
/// system. If you want polar coordinates just ignore the height component of
/// the cylindrical coordinates.
/// 
/// It is important to note that the components of polar and grid coordinates
/// represent the radius, radian angle and height. Which component represents
/// what depends on the gridPlane. The user manual has a handy table for that
/// purpose.
/// 
/// The members <see cref="size"/>, <see cref="renderFrom"/> and <see
/// cref="renderTo"/> are inherited from <see cref="GFGrid"/>, but slightly
/// different. The first component of all three cannot be lower than 0 and in
/// the case of <see cref="renderFrom"/> it cannot be larger than the first
/// component of @c #renderTo, and vice-versa.  The second component is an
/// angle in radians and wraps around as described in the user manual, no other
/// restrictions. The third component is the height, it’s bounded from below by
/// 0.01 and in the case of <see cref="renderFrom"/> it cannot be larger than
/// <see cref="renderTo"/> and vice-versa.
public class GFPolarGrid : GFLayeredGrid {
	...
	/// The radius of the innermost circle and how far apart the other circles
	/// are. The value cannot go below 0.01.
	...
	/// The amount of sectors the circles are divided into. The minimum values
	/// is 1, which means one full circle.
	public int sectors { get {return _sectors;}
		set {
	...
	/// Unity's GL class can only draw straight lines, so in order to get the
	/// sectors to look round this value breaks each sector into smaller
	/// sectors. The number of smoothness tells how many segments the circular
	/// line has been broken into. The amount of end points used is smoothness
	/// + 1, because we count both edges of the sector.
	...
	/// This is a read-only value derived from @c #sectors. It gives you the
	/// angle within a sector in radians and it’s a shorthand writing for
	...
	/// cref="radius"/>, the second one the sector and the thrid one the
	...
	/// <param name="gridPoint">Point in grid space.</param>
	/// 
	/// Converts a point from grid to polar space. The main difference is that
	/// grid coordinates are dependent on the grid's parameters, while polar
	/// coordinates are not.
	...
	/// <param name="polarPoint">Point in polar space.</param>
	/// 
	/// Converts a point from polar to grid space. The main difference is that
	/// grid coordinates are dependent on the grid's parameters, while polar
	/// coordinates are not.
	...
	/// sector has an agle of 360° / 6 = 60°, so a 135° angle corresponds to a
	...
	/// the grid it wraps around, nagetive sctors are automatically subtracted
	/// from the maximum.
	/// 
	/// <example>Let's take a grid with six sectors for example, then one
	/// sector has an agle of 360° / 6 = 60°, so a 2.25 sector corresponds to
	...
	/// 
	/// This is basically the same as <see cref="Angle2Rotation"/>, excpet with
	/// sectors, which makes this method more suitable for grid coordinates
	...
	/// <param name="worldPoint">Point in world space.</param>
	/// 
	/// This method compares the point's position in world space to the grid
	/// and then returns which rotation an object should have if it was at that
	/// position and rotated around the grid.
	...
	/// <param name="worldPoint">Point in world space.</param>
	/// 
	/// This method returns the distance of a world point from the grid's
	/// radial axis. This is not the same as the point's distance from the
	/// grid's origin, because it doesn't take "height" into account. Thus it
	/// is always less or equal than the distance from the origin.
	...
	/// will be drawn at that position.
	...
	/// vertices. If <paramref name="doDebug"/> is set a small gizmo face will
	/// drawn there.
	...
	/// <param name="worldPoint">Point in world space.</param>
	/// 
	/// Similar to <see cref="NearestVertexP"/>, it returns the polar
	/// coordinates of a face on the grid. Since the face is enclosed by
	/// several vertices, the returned value is the point in between all of the
	/// vertices.
	...
	/// the grid. This method cobines two steps in one call for convenience.
	...
	/// might add an optionfor this in the future.
	...
	protected override void DrawPointsCalculate(Vector3[][][] points, ref int[] amount, Vector3 from, Vector3 to) {
		...
		/* How to fill the arc segment line array: Each segment consists of two
 		 ...
 		 * point of the next segment. The very first point will be the the
 		 * first point of the very first segment (segment 0). For every other
 		 * segment (segment i) add only its first point as its first point and
 		 * as the second point of the previous segment (segment i-1). The very
 		 * last point will be added as the second point of the very last
 		 * segment (segment n-1). 
		 */
		...
		/* TO DO: performance can be improved by reducing the amount of
 		 ...
 		 * handling over- and underflow ina  loop without repeating; in the
